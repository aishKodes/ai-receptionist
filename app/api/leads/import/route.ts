...
export function GET() {
  const history = getSqlite().prepare("SELECT id,file_name AS fileName,status,total_rows AS totalRows,imported_rows AS importedRows,skipped_rows AS skippedRows,error_rows AS errorRows,created_at AS createdAt,completed_at AS completedAt FROM lead_imports ORDER BY created_at DESC LIMIT 20").all();

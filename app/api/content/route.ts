...
export async function POST(request: NextRequest) {
  try {
    ...
    getSqlite().prepare("INSERT OR REPLACE INTO content_items (id,type,title,description,url,thumbnail_url,treatment_slug,tags_json,when_to_send,priority,active,approved_for_ai,created_at) VALUES (?,?,?,?,?,NULL,?,?,?,?,?,?,COALESCE((SELECT created_at FROM content_items WHERE id=?),?))").run(id, item.type, item.title, item.description, item.url, item.treatmentSlug ?? null, JSON.stringify(item.tags), item.whenToSend, item.priority, Number(item.active), Number(item.approvedForAi), id, nowIso());

    ...
    {selected && patient ? <>
      ...
      <aside className="intelligence-panel">
        <div className="intel-scroll">
          ...
          <div className="intel-grid"><div><span>Treatment</span><strong>{titleCase(patient.treatmentSlug)}</strong></div><div><span>Readiness</span><strong>{String(selected.state.readinessScore || 0)} / 100</strong></div><div><span>Phase</span><strong>{titleCase(String(selected.state.conversationPhase || "DISCOVERY"))}</strong></div><div><span>Objection</span><strong>{titleCase(String(selected.state.primaryObjection || "None"))}</strong></div><div><span>Language</span><strong>{titleCase(String(selected.state.preferredLanguage || "AUTO"))}</strong></div><div><span>AI status</span><strong>{patient.aiEnabled ? "Active" : "Paused"}</strong></div></div>
          <div className="intel-section"><span className="section-label">NEXT BEST ACTION</span><p>{titleCase(String(selected.state.nextBestAction || "ANSWER"))}</p><small>{String(selected.state.nextActionReason || selected.state.readinessReason || "Continue the conversation naturally")}</small></div>

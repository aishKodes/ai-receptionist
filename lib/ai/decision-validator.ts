...
export function shouldUseFallback(decision: ReceptionDecision, message: string, threshold = Number(process.env.AI_CONFIDENCE_THRESHOLD || 0.58)) {
  const multilingualOrComplex = /[^\u0000-\u007f]/.test(message) || message.length > 900;
  return decision.intentConfidence < threshold || (decision.intent === "unknown" && message.trim().split(/\s+/).length > 5) || multilingualOrComplex;
}

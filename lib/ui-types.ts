export type Patient = {
  ...
  leadScore: number; leadTemperature: "HOT" | "WARM" | "COLD"; leadStage: string; source: string; aiSummary?: string | null;
...
export type ContentItem = { id: string; type: "youtube" | "website" | "before_after" | "faq"; title: string; description: string; url: string; thumbnailUrl?: string | null; treatmentSlug?: string | null; tagsJson?: string; tags?: string[]; whenToSend: string; priority: number; active: number | boolean; createdAt: string };
...
export type DashboardState = {
  patients: Patient[];
  selected: null | { patient: Patient; conversation: Record<string, unknown>; messages: Message[]; appointment: Appointment | null; events: AIEvent[]; sentContent: ContentItem[] };
